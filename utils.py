...
def get_reduced(tensor, current_device, dest_device, world_size):
    ...
    """
    将不同GPU上的变量或tensor集中在主GPU, 并得到均值
    参考https://pytorch.org/tutorials/intermediate/dist_tuto.html#collective-communication
    """
...
def mlm_accuracy(logits: torch.Tensor, labels: torch.Tensor, ignore_index = -100):
    """
    logits
    """
    mlm_pred = logits.argmax(dim = -1).reshape(-1)
    mlm_true = labels.reshape(-1)
    mlm_acc = mlm_pred.eq(mlm_true)
    mask = torch.logical_not(mlm_true.eq(ignore_index))
    mlm_acc = mlm_acc.logical_and(mask)
    correct_num = mlm_acc.sum().item()
    total = mask.sum().item()
    accuracy = float(correct_num) / total
