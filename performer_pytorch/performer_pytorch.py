...
class TokenEmbedding(nn.Module):
    ...
    bins: 1,2,3,...     freeze
    """
    def __init__(self, hidden_size: int = 256, num_bins: int = 5) -> None:
        super().__init__()
        pe = torch.zeros(num_bins, hidden_size)
        position = torch.arange(0.0, num_bins).unsqueeze(1)
        ...
        fixed_embedding = torch.cat([zeros, pe])
        self.fixed_embedding = nn.Embedding.from_pretrained(fixed_embedding)
        self.mask_embedding = nn.Embedding(2, hidden_size, padding_idx = 0)
        self.mask_id = 0
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        fixed_embed = self.fixed_embedding(x)
        x_mask = (x == self.mask_id).long()
        mask_embed = self.mask_embedding(x_mask)

        return fixed_embed + mask_embed

class PerformerLM(nn.Module):
    def __init__(
        self,
        *,
        # 词汇个数, 我这里换成了num_bins, 因此词汇个数为num_bins+1
        ...
        #-----------------------------------------------------------#
        #   这里是scBERT原来写的embedding，可以替换成自己的embedding
        #-----------------------------------------------------------#
        ...
        #     self.layer_pos_emb = Always(None)
        #-----------------------------------------------------------------------#
        self.embedding = TokenEmbedding(num_bins = num_bins, hidden_size = dim)
        # 这个层不知道什么作用，是跟performer有关的模块，因此复制了过来
        self.layer_pos_emb = Always(None)
        #-----------------------------------------------------------------------#
        ...
        #-----------------------------------------------------------#
        #   预训练任务的头, num_tokens换成了num_bins+1
        #-----------------------------------------------------------#
        # self.to_out = nn.Linear(dim, num_tokens) if not tie_embed else None
        #-----------------------------------------------------------------------#
        self.to_out = nn.Linear(dim, num_bins + 1)
        #-----------------------------------------------------------------------#
